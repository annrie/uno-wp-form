...
class Unomoon_Form_Validation_Rule_Required extends Unomoon_Form_Abstract_Validation_Rule {
	...
	public function admin( $key, $value ) {
		?>
		<label><input type="checkbox" <?php checked( $value[ $this->get_name() ], 1 ); ?> name="<?php echo Unomoon_Form_Config::NAME; ?>[validation][<?php echo $key; ?>][<?php echo esc_attr( $this->get_name() ); ?>]" value="1" /><?php esc_html_e( 'No empty( with checkbox )', 'unomoon-form' ); ?></label>
		<?php
	}
}
