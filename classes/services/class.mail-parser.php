...
/**
 * Uno_WP_Form_Mail_Parser
 */
class Uno_WP_Form_Mail_Parser {
	 ...
	 */
	public function __construct( Uno_WP_Form_Mail $Mail, Uno_WP_Form_Setting $Setting ) {
		$this->Mail    = $Mail;
	...
	public function save() {
		...
		$saved_mail_id = wp_insert_post(
			array(
				...
				'post_status' => 'publish',
				'post_type'   => UWF_Functions::get_contact_data_post_type_from_form_id( $form_id ),
			)
		);
		...
		if ( ! empty( $saved_mail_id ) ) {
			...
			UWF_Functions::save_attachments_in_media(
				$saved_mail_id,
	...
	protected function _save( $value ) {
		$Parser  = new Uno_WP_Form_Parser( $this->Setting );
		$matches = Uno_WP_Form_Parser::search( $value );
		...
		foreach ( $matches[1] as $name ) {
			...
			$ignore_keys = apply_filters( 'unoform_no_save_keys_' . $form_key, array() );
			if ( in_array( $name, $ignore_keys, true ) ) {
				continue;
			}

			// ファイルは UWF_Functions::save_attachments_in_media() で ID が保存されるため
		...
		$data = array_merge(
			array(
				'admin_mail_to' => $this->Mail->to, // admin_mail_to = The property of Uno_WP_Form_Contact_Data_Setting
			),
			$data
		);

		$Contact_Data_Setting = new Uno_WP_Form_Contact_Data_Setting( $this->Data->get_saved_mail_id() );
